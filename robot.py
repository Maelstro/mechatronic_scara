...
class Robot:
    ...
    def move_manual_cartesian(self, key=None, enable=None):
        if enable is not None:
            if enable:
                self.write_port(6, [])
            else:
    ...
    def move_square(self, x_1, y_1, x_n, y_n):
        return self.write_port(4, [x_1, y_1, x_n, y_n])

    def move_circle(self, x, y, radius):
        return self.write_port(8, [radius, x, y])

    def draw_circle_in_a_square(self, x_1, y_1, x_n, y_n):
        return self.write_port(9, [x_1, y_1, x_n, y_n])
